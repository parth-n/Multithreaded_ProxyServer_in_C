...
#define MAX_BYTES 4096 //max size of request/response
#define MAX_ELEMENT_SIZE 10*(1<<10) //max size of cache element
#define MAX_SIZE 200*(1<<20) //max size of cache
...
cache_element* cache_map = NULL; // hash map for cache elements

cache_element* find(char* url);
int add_cache_element(char* data,int size,char* url);
void remove_cache_element();

int port_number = 1902;				// Default Port
...
cache_element* head ; //head of Lru cache linkedlist .
int cache_size ;  //size of cache


int connectRemoteServer(char* host_addr, int port_num) { //function to connect to remote server
    int remoteSocket = socket(AF_INET, SOCK_STREAM, 0); //creating a socket
    if (remoteSocket < 0) {                    //checking if socket is created or not
        perror("Error in Creating Socket");
        return -1;
    }

    struct hostent *host = gethostbyname(host_addr); //getting the host by name
    if (host == NULL) {
    ...
    memcpy((char*)&server_addr.sin_addr.s_addr, (char*)host->h_addr, host->h_length); //copying the host address
    server_addr.sin_port = htons(port_num); //converting the port number to network byte order

    if (connect(remoteSocket, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) { //connecting to the remote server
...
int handle_request(int clientsocketID, struct ParsedRequest *request, char* tempReq) { //function to handle the request
    char* buf = (char*)malloc(sizeof(char) * MAX_BYTES); //allocating memory to buffer
    if (buf == NULL) {
    ...
    if (ParsedHeader_get(request, "Host") == NULL) { //getting the host header key
        if (ParsedHeader_set(request, "Host", request->host) < 0) { //setting the host header key
            printf("Set \"Host\" header key not working\n");
        }
    }

    if (ParsedRequest_unparse_headers(request, buf + strlen(buf), (size_t)MAX_BYTES - strlen(buf)) < 0) { //unparsing the headers
        printf("unparse failed\n");
    }

    int server_port = 80; // default remote server port (not the proxy server, but the end server)
    if (request->port != NULL) {
        server_port = atoi(request->port);
    }
    int remotesocketID = connectRemoteServer(request->host, server_port); // connecting to the remote server
    if (remotesocketID < 0) {
        free(buf);
        return -1;
    }
    int bytes_send = send(remotesocketID, buf, strlen(buf), 0); // sending the request to the remote server
    bzero(buf, MAX_BYTES);
    bytes_send = recv(remotesocketID, buf, MAX_BYTES - 1, 0); // receiving the response from the remote server
    ...
    while (bytes_send > 0) {
        int bytes_sent_to_client = send(clientsocketID, buf, bytes_send, 0); // sending the response to the client
...
void *thread_fn(void *socketNew){ //thread function
    sem_wait(&semaphore) ; //semaphore wait
    int p ;
    sem_getvalue(&semaphore,&p) ; //get semaphore value
    printf("Semaphore value : %d\n",p) ;
    int *t = (int*)(socketNew) ;//typecasting the socketNew to int
    int socket = *t ;//socket is socket descriptor of the connected client
    int bytes_send_client, len ;//bytes_send_client is the bytes transferred, len is the length of the buffer.
    char *buffer = (char*)calloc(MAX_BYTES,sizeof(char)) ;//creating buffer of 4kb for a client
    
    bzero(buffer,MAX_BYTES) ;//making buffer zero/removing garbage values
    ...
    while(bytes_send_client>0){
        len = strlen(buffer) ;
        if(strstr(buffer,"\r\n\r\n")==NULL){
            bytes_send_client = recv(socket,buffer+len,MAX_BYTES-len,0) ; //loop until u find "\r\n\r\n" in the buffer
        }
        else{
            break ;
        }
    ...
    char *tempReq = (char*)malloc(strlen(buffer)*sizeof(char)+1) ;//tempReq, buffer both store the http request sent by client
    for(int i=0 ;i< strlen(buffer) ;i++){
        tempReq[i]=buffer[i] ;
    }
    cache_element* temp = find(tempReq) ;//checking for the request in cache
    if(temp!=NULL){
        int size = temp->len/sizeof(char) ;
        int pos = 0 ; 
        char response[MAX_BYTES] ; //response to be sent to the client
        while(pos<size){
        ...
        len=strlen(buffer) ;
        struct ParsedRequest* request = ParsedRequest_create() ;//parsing the request
    ...
    shutdown(socket,SHUT_RDWR) ; //shutdown the socket
    close(socket) ; //close the socket
    free(buffer) ; //free the buffer (heap m/y)
    sem_post(&semaphore) ; //semaphore post/signal
    printf("Semaphore post value : %d\n",p) ;//printing the semaphore value
    free(tempReq) ; //free the tempReq (heap m/y)
    return NULL;
}
...
int main(int argc, char* argv[]){
    ...
    sem_init(&semaphore,0,MAX_CLIENTS) ; // semaphore initialsed with max clients ;
    pthread_mutex_init(&lock,NULL) ; // lock initialsed for cache
    ...
    int reuse = 1; // reusing the same proxy socket for multiple requests to same server .
    if(setsockopt(proxy_socketId,SOL_SOCKET,SO_REUSEADDR,(const char*)&reuse,sizeof(reuse))<0){
        perror("setsockopt failed !\n") ;
    }

    bzero((char*)&server_addr,sizeof(server_addr)) ;//cleans the garbage values in the addr.
    server_addr.sin_family = AF_INET ;
    server_addr.sin_port = htons(port_number) ;// converts port no to htons format .
    server_addr.sin_addr.s_addr = INADDR_ANY ; // any available address is assigned to the server.
    ...
    int i=0 ;//pointer to accepted and started client.
    ...
    while(1){
        ...
        struct sockaddr_in* client_ptr = (struct sockaddr_in*)&client_addr ;//copy of client address.
        struct in_addr ip_addr = client_ptr->sin_addr ; //ip address of client.
        char str[INET_ADDRSTRLEN] ; // INET_ADDRSTRLEN: Default ip address size
        inet_ntop(AF_INET,&ip_addr,str,INET_ADDRSTRLEN) ; //converts ip address to string format.
        printf("Client is connected with port number: %d and ip address: %s \n",ntohs(client_addr.sin_port),str) ;//printing the client details.
        pthread_create(&tid[i],NULL,thread_fn,(void*)&connected_socketID[i]) ;//creating a thread for each client accepted.
        i++ ;
    }
    close(proxy_socketId) ; //close the socket
    return 0 ;
}
...
cache_element* find(char* url) {
    ...
    HASH_FIND_STR(cache_map, url, site); // find the cache element in the hash map
    if (site != NULL) {
    ...
    temp_lock_val = pthread_mutex_unlock(&lock); // unlock the cache
    if (temp_lock_val != 0) {
        perror("Failed to release lock");
    }
    printf("Cache Lock released %d\n", temp_lock_val);
...
void remove_cache_element() {
    ...
    cache_element *oldest_element = NULL;

    while (p != NULL) {
        if (p->lru_time_track < oldest_time) {
            oldest_time = p->lru_time_track;
            oldest_element = p;
            q = p;
        }
        p = p->next;
    ...
    if (oldest_element != NULL) {
        if (q != NULL) {
            q->next = oldest_element->next;
        } else {
            head = oldest_element->next;
        }
        cache_size -= (oldest_element->len + 1 + strlen(oldest_element->url) + sizeof(cache_element));
        HASH_DEL(cache_map, oldest_element); // remove the element from the hash map
        free(oldest_element->data);
        free(oldest_element->url);
        free(oldest_element);
    }
}
...
int add_cache_element(char* data, int size, char* url) {
    ...
    int element_size = size + 1 + strlen(url) + sizeof(cache_element); // size of the element to be added
    if (element_size > MAX_ELEMENT_SIZE) {
        ...
        while (cache_size + element_size > MAX_SIZE) {
            remove_cache_element(); // remove the cache element
        }
        cache_element* element = (cache_element*)malloc(sizeof(cache_element)); // allocate memory for the element
        if (element == NULL) {
        ...
        if (element->data == NULL) {
            perror("Failed to allocate memory for element data");
            free(element);
            pthread_mutex_unlock(&lock);
            return -1;
        }
        ...
        if (element->url == NULL) {
            perror("Failed to allocate memory for element URL");
            free(element->data);
            free(element);
            pthread_mutex_unlock(&lock);
            return -1;
        ...
        HASH_ADD_STR(cache_map, url, element); // add the element to the hash map
